...
keep_newyork = keep_county[keep_county["state"] == "New York"]

# Get a set of Alabama county names for efficient lookup
newyork_counties = set(keep_newyork["county"])

# Keep only rows in df_out where 'County' is in that list
df_out6 = df_out[df_out["County"].isin(newyork_counties)]

# Save the final DataFrame to a CSV file
df_out6.to_csv("df_6.csv", index=False)